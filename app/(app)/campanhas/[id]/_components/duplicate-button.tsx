...
export function DuplicateButton({ campaignId }: { campaignId: string }) {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <button disabled={pending || !anchor} onClick={() => startTransition(async () => { const id = await duplicateCampaignAction(campaignId, anchor, ""); router.push(`/campanhas/${id}`); })} className="rounded-lg bg-emerald hover:bg-emeraldd text-white text-sm font-semibold px-3 py-1.5 disabled:opacity-50">{pending ? "Duplicando…" : "Duplicar nesta data"}</button>
